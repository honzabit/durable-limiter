...
export async function handleRequest(request: Request, env: Bindings, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
        const id = env.RATE_LIMITER.idFromName(requestJson.key);
        const rateLimiter = env.RATE_LIMITER.get(id);

        const cache = await caches.open('durable-limiter');
